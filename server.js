...
let serviceAccount = require("./osproto-8297d-firebase-adminsdk-5yxfo-aa600e095e.json");
...
app.post('/', (req, res) => {
    ...
    db.collection('notify').doc(email).get()
        .then(user => {
            if (user.exists) {
                return res.json({ 'alert': 'email already exists' });
            } else {
                db.collection('notify').doc(email).set(req.body)
                    .then(data => {
                        res.json({
                            name: req.body.name,
...
app.post('/seller', (req, res) => {
    ...
    if (!name.length || !address.length || !about.length || number.length < 10 || !Number(number)) {
        return res.json({ 'alert': 'some informatio(s) is/are invalid' })
    } else if (!tac || !legit) {
        return res.json({ 'alert': 'you must agree to our terms and conditions' });
    } else {
...
app.post('/urlt', (req, res) => {

    let { twtLink } = req.body;

    
                db.collection('socials').doc('social-links').update({
                    twitter : twtLink
                })
})
app.post('/urli', (req, res) => {
...
app.get('/urlIn', (req, res) => {
    const ref = db.collection('socials').doc('social-links');
...
app.post('/add-product', (req, res) => {
    ...
    if (!draft) {
        if (!name.length) {
            ...
            return res.json({ 'alert': 'short description must be between 10to100 letters long' });
        } else if (!des.length) {
